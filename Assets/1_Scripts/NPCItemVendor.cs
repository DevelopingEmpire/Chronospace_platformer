...
public class NPCItemVendor : MonoBehaviour
{
    ...
    public float conversionSize = 1f;
    ...
    void Start()
    {
        ...
        if(objectExampleSpawnPos.GetComponent<Item>()) {
            ...
            CharacterController characterController = objectExampleSpawnPos.GetComponent<CharacterController>();
            if (characterController) {
                characterController.enabled = false; // CharacterController 비활성화
            }
            objectExampleSpawnPos.transform.localScale = new Vector3(conversionSize, conversionSize, conversionSize);
        }
    }
